...
for filename in os.listdir(path):
    ...
    if filename.endswith(".csv"):
        ...
        if all(col in df.columns for col in ['Date', 'Open', 'High', 'Low', 'Close']):
            ...
            # Calculate the RSI and add it as a new column to the DataFrame
            close_prices = df['close'].values
            ...
            if df["close"].iloc[-1] > ema[-1] and talib.SMA(ema, timeperiod=10)[-1] > 0:
                # Check if the RSI is in an uptrend and within the desired range
                rsi_trend = rsi[-1] > rsi[-2]
                rsi_range = 40 <= rsi[-1] <= 65
                if rsi_trend and rsi_range:
                    # Plot the ATR, EMA, and RSI
                    fig, ax = plt.subplots()
                    ax.plot(df.index, df['atr'])
                    ax.plot(df.index, df['ema'])
                    ax.plot(df.index, df['rsi'])
                    ax.axhline(y=40, color='red')
                    ax.axhline(y=65, color='red')
